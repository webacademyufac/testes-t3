...
//faz-se uma apresentação, um arranjo
describe('/containers/collection', () => {
  describe('Deve exibir a tela de coleções corretamente quando nenhuma coleção for injetada', () => {
    it('Deve exibir o título da página quando ela for carregada', async () => {
      ...
      await act(async () => render(<Collection collectionService={collectionService} />)); //envio, uma ação

      //retorno da comparação, um arranjo
      expect(screen.getByRole('heading', { name: 'Citei' })).toBeInTheDocument(); //passa o texto citei esperando q seja retornado algo
    });
  ...
  describe('Deve filtrar as coleções corretamente', () => {
    ...
    it('Deve esconder as coleções cujo título não é igual ao título digitado no campo de busca', async () => {
      const collections = Array.from(Array(faker.number.int({ min: 4, max: 10 }))).map(() =>
        collection({
          title: faker.word.words(2), //gera numero aleatorio entre 4 e 10. mapeando e para cada colection gera um titulo aleatorio
        })
      );
      const [first, second] = faker.helpers.arrayElements(collections, 2); //sorteia dois elementos
      const collectionService = collectionServiceFactory({  
        getValidCollections: jest.fn().mockResolvedValue(collections), //retorna o array de coleções aleatórias
        filterCollections: (name, collections) =>
          collections.filter(collection => collection.title === name),
      });
      await act(async () => render(<Collection collectionService={collectionService} />)); //renderiza
      await userEvent.click(screen.getByLabelText('search-icon')); //evento

      await userEvent.type(screen.getByRole('textbox'), first.title);

      expect(screen.queryByText(second.title)).toBe(null); //espera-se q seja elemento vazio
    });
  });
});
