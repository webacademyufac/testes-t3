...
describe('<Quotation>', () => {
  describe('Deve deixar de exibir as informações quando elas não forem passadas para o componente', () => {//Ele verifica se o parágrafo da citação não está sendo renderizado quando nenhum valor é passado para o atributo quote.
    it('Deve deixar de renderizar o parágrafo da citação quando nada for passado no atributo "quote"', () => {
      render(<Quotation />);

      expect(screen.getByTitle('quote-title')).not.toBeInTheDocument();//a função getByTitle do screen é usada para selecionar os elementos pelos seus atributos "title" e, em seguida, o que foi usado é a função not.toBeInTheDocument() para verificar que esses elementos não estão presentes na tela
    });
