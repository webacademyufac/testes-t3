...
beforeAll(() => {//A função beforeAll é usada para criar um mock da função alert do objeto window. Isso é feito para evitar que as chamadas reais de alert afetem os testes.
...
//os testes verificam se o título e o subtítulo da página são ocultados quando o ícone de busca é clicado.
  ...
  describe('Deve renderizar as coleções corretamente quando elas forem injetadas via prop', () => {//Esse teste verifica se as coleções fornecidas por meio do serviço de coleções são exibidas corretamente no componente.
  ...
  describe('Deve exibir corretamente o modal de adicionar coleção', () => {//Esses testes verificam se o modal de adicionar coleção é exibido corretamente quando o botão "Adicionar coleção" é clicado.
  ...
  describe('Deve filtrar as coleções corretamente', () => {//Esses testes verificam se as coleções são filtradas corretamente com base no texto digitado no campo de busca.
