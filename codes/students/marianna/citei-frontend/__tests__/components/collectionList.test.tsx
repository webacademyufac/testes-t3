...
describe('<CollectionList>', () => {
  describe('Deve renderizar uma lista de coleções em branco', () => {
    it('Deve exibir um elemento em branco quando uma lista de coleções vazia for injetada via prop', () => {
      ...
      expect(container.firstChild).toBeEmptyDOMElement();       // Verifica se o primeiro elemento do container está vazio
    });
    ...
    it('Deve exibir corretamente o nome de todas as coleções passadas via prop', () => {
      ...
      collections.forEach(({ title }) => expect(screen.getByText(title)).toBeInTheDocument());       // Verifica se o nome de cada coleção está sendo exibido corretamente
    });
    ...
    it('Deve renderizar o mesmo numero de coleções passadas via prop "collections"', () => {
      ...
      expect(container.firstChild?.childNodes.length).toBe(collections.length);       // Verifica se o número de elementos filhos do container é igual ao número de coleções passadas
    });
  });
});